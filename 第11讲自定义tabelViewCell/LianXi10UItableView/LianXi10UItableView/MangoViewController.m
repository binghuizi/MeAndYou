...
//    cell.imageView.image = [UIImage imageNamed:image[indexPath.row]];
    ...
    if ([gender[indexPath.row] isEqualToString:@"女"]) {
        static NSString *girlIdetir = @"girlCell";
        girlTableViewCell *girlCell = [tableView dequeueReusableCellWithIdentifier:girlIdetir];
        if (girlCell == nil) {
            girlCell = [[girlTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:girlIdetir];
            NSMutableArray *name   = self.nameArray[indexPath.section];
            NSMutableArray *number = self.numberArray[indexPath.section];
            NSMutableArray *image  = self.imageArray[indexPath.section];
            
            girlCell.textLabel.text       = name[indexPath.row];
            girlCell.detailTextLabel.text = number[indexPath.row];
            girlCell.imageView.image      = [UIImage imageNamed:image[indexPath.row]];
        }
        return girlCell;
        
    }else {
        static NSString *boyIdetir = @"boylCell";
        boyTableViewCell *boylCell = [tableView dequeueReusableCellWithIdentifier:boyIdetir];
        if (boylCell == nil) {
            boylCell = [[boyTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:boyIdetir];
            NSMutableArray *name   = self.nameArray[indexPath.section];
            NSMutableArray *number = self.numberArray[indexPath.section];
            NSMutableArray *image  = self.imageArray[indexPath.section];
            
            boylCell.textLabel.text       = name[indexPath.row];
            boylCell.detailTextLabel.text = number[indexPath.row];
            boylCell.imageView.image      = [UIImage imageNamed:image[indexPath.row]];
        }
        return boylCell;
    }
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
 //
    ...
    [self.numberArray[indexPath.section] removeObjectAtIndex:indexPath.row];
    [self.imageArray[indexPath.section] removeObjectAtIndex:indexPath.row];
    [self.genderArray[indexPath.section] removeObjectAtIndex:indexPath.row];
    
#pragma mark ------- 3、删除联系人时，如果联系人是某分区最后一条数据时，直接删除整个分区。
    if ( group.count == 0 ) {
        [self.nameArray removeObjectAtIndex:indexPath.section];
        [self.numberArray removeObjectAtIndex:indexPath.section];
        [self.imageArray removeObjectAtIndex:indexPath.section];
        [self.genderArray removeObjectAtIndex:indexPath.section];
      //通知TableView删除对应分区
        [self.tabelView deleteSections:[NSIndexSet indexSetWithIndex:indexPath.section] withRowAnimation:UITableViewRowAnimationFade];
    }else{
        //通知TableView删除对应行
...
//    [gender insertObject:@"男"       atIndex:indexPath.row];
//    //通知TableView添加对应行
...
-(void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath{
    ...
    NSMutableArray *number = self.numberArray[sourceIndexPath.section];
    NSMutableArray *image  = self.imageArray[sourceIndexPath.section];
    NSMutableArray *gender = self.genderArray[sourceIndexPath.section];
    //2.取出当前要移动的元素
    NSString *name     = [group[sourceIndexPath.row] retain];
    NSString *phoneNum = [number[sourceIndexPath.row] retain];
    NSString *imageNS  = [image[sourceIndexPath.row] retain];
    NSString *sex      = [gender[sourceIndexPath.row] retain];
    //3.删除原来的元素
    [group removeObjectAtIndex:sourceIndexPath.row];
    [number removeObjectAtIndex:sourceIndexPath.row];
    [image removeObjectAtIndex:sourceIndexPath.row];
    [gender removeObjectAtIndex:sourceIndexPath.row];
    //4.取出当前元素 然后在插入到 新的位置（也就是移动的位置）
    [group insertObject:name atIndex:destinationIndexPath.row];
    [number insertObject:phoneNum atIndex:destinationIndexPath.row];
    [image insertObject:imageNS atIndex:destinationIndexPath.row];
    [gender insertObject:sex atIndex:destinationIndexPath.row];
}
//监测移动操作 移动不能越界所在的区域

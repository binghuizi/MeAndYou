...
//                 delete from  LinkMans where name= '%@'",name
    NSString * sql = [NSString stringWithFormat:@"delete from LinkMans where name = '%@'",name];
...
-(void)updateLinkManWithPhoneNUmber:(NSString *)phoneNumber withName:(NSString *)name{
    [self openDataBase];
    NSString *sql = [NSString stringWithFormat:@"update LinkMans set phoneNumber = '%@' where name = '%@'",phoneNumber,name];
    if (sqlite3_exec(database, [sql UTF8String], NULL, NULL, NULL) == SQLITE_OK) {
        NSLog(@"根据姓名更新手机号成功");
    }else{
        NSLog(@"根据姓名更新手机号失败");
